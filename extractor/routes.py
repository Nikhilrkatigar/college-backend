...
SERPAPI_KEY = os.getenv("SERPAPI_KEY") or "4f289c8b0be0ee99d6eec09d2a907ae02a780f7c7ebc54271542720dcbd448ab"
...
def process_result(item: dict, city: str, state: str, region: str,
                   college_type: str, done_by: str, location_key: str, district: str = "") -> bool:
    """Process result"""
    try:
...
def extraction_worker(job_id: str, region: str, state: str, district: str, city: str,
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {
                executor.submit(
                    process_result,
                    item, city, state, region, college_type, done_by, location_key, district
...
def run_extraction(
    ...
    Thread(
        ...
        args=(job_id, region, state, district, city, college_type, current_user["username"]),
        daemon=True
    ).start()
